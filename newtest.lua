...
function Library.Window(self, Options)
	...
	function Tabs.Section(self, Properties)
		...
		-- Use the SectionCount to lock the original position in the list
		self.SectionCount = (self.SectionCount or 0) + 1
		local originalOrder = self.SectionCount
		...
		sectionfreme.LayoutOrder = originalOrder -- Lock the order here
		sectionfreme.Size = UDim2.new(1, 0, 0, 0)
		sectionfreme.Parent = self.Elements.Content

		-- [UI Objects remain exactly the same as your style]
		local uIListLayout = Instance.new("UIListLayout")
		...
		-- Improved Dragging with Animation and Order Retention
			...
			if input.UserInputType == Enum.UserInputType.MouseButton1 then
				...
				connection = UIS.InputEnded:Connect(function(EndedInput)
					if EndedInput.UserInputType == Enum.UserInputType.MouseButton1 then
						...
						if
							mPos.X >= cPos.X
							and mPos.X <= cPos.X + cSize.X
							and mPos.Y >= cPos.Y
							and mPos.Y <= cPos.Y + cSize.Y
						then
							-- Create a temporary "proxy" position to animate towards
							-- We calculate where it would be inside the content frame
							local targetPos = UDim2.fromOffset(0, 0) -- UIListLayout will handle actual Y

							local tween = TweenService:Create(
								sectionfreme,
								TweenInfo.new(0.3, Enum.EasingStyle.Quart, Enum.EasingDirection.Out),
								{
									Position = UDim2.fromOffset(cPos.X, absPos.Y), -- Smooth slide to the X column
									Size = UDim2.new(0, cSize.X, 0, absSize.Y), -- Match width of container
								}
							)

							tween:Play()
							tween.Completed:Connect(function()
								if not dragging then -- Ensure we haven't started dragging again
									sectionfreme.Parent = content
									sectionfreme.LayoutOrder = originalOrder -- Return to original spot
									sectionfreme.AutomaticSize = Enum.AutomaticSize.Y
									sectionfreme.Size = UDim2.new(1, 0, 0, 0)
								end
							end)
