...
function Tabs.Section(self, Properties)
    ...
    -- Ensure the Parent Tab's ListLayout respects our order
    ...
    -- [Other UI elements like uIListLayout, uIPadding, uICorner, header, etc. remain the same]
    local uIListLayout = Instance.new("UIListLayout")
    ...
    local function Pulse()
        local originalColor = Color3.fromRGB(65, 62, 67)
        local pulseColor = Color3.fromRGB(255, 255, 255) -- White pulse
        
        -- Quickly flash stroke to white, then fade back
        uIStroke.Transparency = 0
        uIStroke.Color = pulseColor
        
        local tween = TweenService:Create(uIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
            Color = originalColor,
            Transparency = 0.5
        })
        tween:Play()
    ...
    UIS.InputChanged:Connect(function(input)
        if dragging and input.UserInputType == Enum.UserInputType.MouseMovement then
            local delta = input.Position - dragStart
            sectionfreme.Position = UDim2.new(0, startPos.X.Offset + delta.X, 0, startPos.Y.Offset + delta.Y)
        end
    end)
