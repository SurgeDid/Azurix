...
function Tabs.Section(self, Properties)
    ...
    -- 1. TRACK ORIGINAL POSITION
    self.SectionCount = (self.SectionCount or 0) + 1
    local myOriginalOrder = self.SectionCount
    ...
    sectionfreme.Size = UDim2.new(1, 0, 0, 0)
    -- Explicitly set LayoutOrder so it knows where it belongs
    sectionfreme.LayoutOrder = myOriginalOrder 
    sectionfreme.Parent = self.Elements.Content

    -- Ensure the Tab's ListLayout is actually sorting by LayoutOrder
    local contentLayout = self.Elements.Content:FindFirstChildOfClass("UIListLayout")
    ...
    sectionname.FontFace = Font.new("rbxassetid://12187365364", Enum.FontWeight.Medium)
    ...
    header.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            ...
            local absSize = sectionfreme.AbsoluteSize
            local absPos = sectionfreme.AbsolutePosition
            
            -- Lock size and detach
            sectionfreme.AutomaticSize = Enum.AutomaticSize.None
            sectionfreme.Size = UDim2.fromOffset(absSize.X, absSize.Y)
            sectionfreme.Position = UDim2.fromOffset(absPos.X, absPos.Y)
            sectionfreme.Parent = Library.ScreenGUI
            
            startPos = sectionfreme.Position

            local connection
            connection = UIS.InputEnded:Connect(function(EndedInput)
                if EndedInput.UserInputType == Enum.UserInputType.MouseButton1 then
                    ...
                    local cPos, cSize = content.AbsolutePosition, content.AbsoluteSize
                    
                    -- Check if released over Tab Content
                    if mPos.X >= cPos.X and mPos.X <= cPos.X + cSize.X and
                       mPos.Y >= cPos.Y and mPos.Y <= cPos.Y + cSize.Y then
                        
                        -- 1. Visual Animation
                        local snapTween = TweenService:Create(sectionfreme, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
                            Position = UDim2.fromOffset(cPos.X + (cSize.X/2) - (absSize.X/2), cPos.Y + (cSize.Y/2)),
                            Size = UDim2.new(0, cSize.X, 0, absSize.Y)
                        })
                        
                        snapTween:Play()
                        
                        -- 2. Reparent and fix Order
                        task.wait(0.1) -- Short delay so the user sees the movement begin
                        sectionfreme.Parent = content
                        sectionfreme.LayoutOrder = myOriginalOrder -- Force it back to original slot
                        sectionfreme.AutomaticSize = Enum.AutomaticSize.Y
                        sectionfreme.Size = UDim2.new(1, 0, 0, 0)
                    end
                end
    ...
    UIS.InputChanged:Connect(function(input)
        if dragging and input.UserInputType == Enum.UserInputType.MouseMovement then
            local delta = input.Position - dragStart
            sectionfreme.Position = UDim2.new(
